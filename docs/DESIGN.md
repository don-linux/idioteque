...
Terminal (`/configuracion/terminal`): fuente del sistema (dropdown; se filtra escribiendo en el panel abierto), tamaño de 10 a 24 píxeles con +/−, un selector de tema (Tokyo Night y otras paletas oficiales), y una vista previa compacta con xterm.js real (`❯ git status`) más la paleta ANSI. El xterm de esa miniatura no se usa; solo se ve. Al elegir tema se recarga; el tamaño de fuente se refleja al momento.
