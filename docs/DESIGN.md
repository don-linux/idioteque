...
Clic derecho en el explorador no muestra el menú del browser. En una fila de archivo o carpeta abre el menú de idioteque: Borrar (Delete) y Renombrar (F2). En el header, la toolbar o el vacío no aparece nada. Esas teclas solo se escriben dentro de ese menú; en el resto del workspace no se anuncian. Con la fila enfocada, F2 renombra en el sitio y Delete borra, con confirmación. Delete también funciona en carpetas: se va la carpeta y todo lo que tenga adentro. El basurero al pasar el mouse sigue en los archivos. Renombrar es el mismo campo inline de crear, con el nombre actual; no se puede mover a otra carpeta con `/`. Todo eso escribe directo al disco.
